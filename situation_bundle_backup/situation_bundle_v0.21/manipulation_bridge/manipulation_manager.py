...
2) For Initialize flows, after Stand completes, publish ``/perception/pick_check_request`` and handle
   ...
   same handling. Perception always ends with ``/manipulation/state`` **SUCCESS** (payload includes
   ``can_pick``, ``delta``, ``position``). ``delta`` / ``position`` are forwarded whether
   ``can_pick`` is true or false. If ``can_pick`` is false, the next command with the same
...
5) **G1 manipulation chains** (e.g. ``G1Task1-010-Manipulation``, ``G1Task2-010-Manipulation``): publishes
   ...
   then after execution ``SUCCESS``, ``…-Box``; ends with ``/manipulation/state`` **SUCCESS**. Executing
...
# Perception step IDs differ by task line; SUCCESS payload (can_pick, delta, position) is the same.
