...
2) For Initialize flows, after Stand completes, publish ``/perception/pick_check_request`` and handle
   ...
   ``VegaTask2-012-Perception``, ``VegaTask3-012-Perception``, or ``VegaTask4-012-Perception`` (per Initialize variant); all use the
...
4) When ``can_pick`` is true, ``position`` from perception is stored per task line until the
   ...
   ``VegaTask4-021-Grab``. The first ``/situation/next_action`` for that Grab includes ``position``,
...
class ManipulationManager(Node):
    def __init__(self) -> None:
        ...
        # Perception (can_pick true) → later Grab on next_action (one line per VegaTask{1,2,3,4}).
    ...
    def _on_perception_result(self, msg: String) -> None:
        ...
        if active_op in INITIALIZE_PERCEPTION_WORKFLOWS:
            ...
            # VegaTask1/2/3/4 Initialize perception share this path (012 step names in message).
            if can_pick:
                self.publish_state(
                    active_sid,
                    "success",
