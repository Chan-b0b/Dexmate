...
2) For Initialize flows, after Stand completes, publish ``/perception/pick_check_request`` and handle
   ...
   ``VegaTask2-012-Perception``, ``VegaTask3-012-Perception``, ``VegaTask4-012-Perception``, or
   ``VegaTask5-012-Perception`` (per Initialize variant); all use the
...
4) When ``can_pick`` is true, ``position`` from perception is stored per task line until the
   ...
   ``VegaTask2-021-Grab``; ``VegaTask3-012`` → ``VegaTask3-020-Manipulation`` /
   ``VegaTask3-021-Grab``; ``VegaTask4-012`` → ``VegaTask4-020-Manipulation`` /
   ``VegaTask4-021-Grab``; ``VegaTask5-012`` → ``VegaTask5-020-Manipulation`` /
   ``VegaTask5-021-Grab``. The first ``/situation/next_action`` for that Grab includes ``position``,
...
WORKFLOW_STEPS: dict[str, list[str]] = {
    ...
    "VegaTask3-020-Manipulation": ["VegaTask3-021-Grab"],
    "VegaTask3-040-Finish": ["VegaTask3-041-Place", "VegaTask3-042-Sit"],
    "VegaTask4-020-Manipulation": ["VegaTask4-021-Grab"],
    "VegaTask4-040-Finish": ["VegaTask4-041-Place", "VegaTask4-042-Sit"],
    "VegaTask5-020-Manipulation": ["VegaTask5-021-Grab"],
    "VegaTask5-040-Finish": ["VegaTask5-041-Place", "VegaTask5-042-Sit"],
}
...
INITIALIZE_PERCEPTION_WORKFLOWS: dict[str, tuple[str, str]] = {
    ...
    "VegaTask3-010-Initialize": ("VegaTask3-011-Stand", "VegaTask3-012-Perception"),
    "VegaTask4-010-Initialize": ("VegaTask4-011-Stand", "VegaTask4-012-Perception"),
    "VegaTask5-010-Initialize": ("VegaTask5-011-Stand", "VegaTask5-012-Perception"),
}
...
VEGA_TASK_IDS: frozenset[str] = frozenset(
    (
        ...
        "VegaTask2-042-Sit",
        "VegaTask3-011-Stand",
        "VegaTask3-012-Perception",
        "VegaTask3-021-Grab",
        "VegaTask3-041-Place",
        "VegaTask3-042-Sit",
        "VegaTask4-011-Stand",
        "VegaTask4-012-Perception",
        "VegaTask4-021-Grab",
        "VegaTask4-041-Place",
        "VegaTask4-042-Sit",
        "VegaTask5-011-Stand",
        "VegaTask5-012-Perception",
        "VegaTask5-021-Grab",
        "VegaTask5-041-Place",
        "VegaTask5-042-Sit",
        "G1Task1-1",
...
class ManipulationManager(Node):
    def __init__(self) -> None:
        ...
        # Perception (can_pick true) → later Grab on next_action (one line per VegaTask{1,2,3,4,5}).
        ...
        self._vega_task4_pick_position: dict[str, Any] | None = None
        self._vega_task5_pick_position: dict[str, Any] | None = None
    ...
    def _start_workflow(self, operation_type: str, sequence_id: int) -> tuple[bool, str]:
        ...
        if operation_type == "VegaTask1-020-Manipulation":
            ...
            if pos is not None:
                extra_payload = {"position": dict(pos)}
        elif operation_type == "VegaTask3-020-Manipulation":
            with self._lock:
                pos = self._vega_task3_pick_position
            if pos is not None:
                extra_payload = {"position": dict(pos)}
        elif operation_type == "VegaTask4-020-Manipulation":
            with self._lock:
                pos = self._vega_task4_pick_position
            if pos is not None:
                extra_payload = {"position": dict(pos)}
        elif operation_type == "VegaTask5-020-Manipulation":
            with self._lock:
                pos = self._vega_task5_pick_position
            if pos is not None:
                extra_payload = {"position": dict(pos)}
        ...
        if ok and extra_payload is not None:
            with self._lock:
                if operation_type == "VegaTask1-020-Manipulation":
                    ...
                    self._vega_task4_pick_position = None
                elif operation_type == "VegaTask5-020-Manipulation":
                    self._vega_task5_pick_position = None
            ...
            with self._lock:
                ...
                if perception_step == "VegaTask1-012-Perception":
                    ...
                    if can_pick and position is not None:
                        ...
                        self._vega_task2_pick_position = None
                elif perception_step == "VegaTask3-012-Perception":
                    if can_pick and position is not None:
                        self._vega_task3_pick_position = dict(position)
                    else:
                        self._vega_task3_pick_position = None
                elif perception_step == "VegaTask4-012-Perception":
                    if can_pick and position is not None:
                        self._vega_task4_pick_position = dict(position)
                    else:
                        self._vega_task4_pick_position = None
                elif perception_step == "VegaTask5-012-Perception":
                    if can_pick and position is not None:
                        self._vega_task5_pick_position = dict(position)
                    else:
                        self._vega_task5_pick_position = None

            # VegaTask1/2/3/4/5 Initialize perception share this path (012 step names in message).
            if can_pick:
                self.publish_state(
                    active_sid,
                    "success",
