...
FENV=(FILM_COND=contact,fz,seal FILM_MASK_FORCE=1 FILM_F0=6 FILM_TAU=4
      FILM_FZ_TAU=5 FILM_FZ_OFF="$FZ_OFF" FILM_INJECT="$INJECT" CUDA_VISIBLE_DEVICES="$GPU")
...
mkdir -p "$DIR/probes"
echo "[$TAG] kind=$KIND inject=$INJECT gpu=$GPU ckpt=$CK"
...
state pc_fc  --swap firstcontact --pre-contact 10
state ramp8  --swap fcscale --fc-mag 8
state ramp10 --swap fcscale --fc-mag 10
state ramp12 --swap fcscale --fc-mag 12
