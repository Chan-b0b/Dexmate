#!/usr/bin/env bash
# 0729 FROMNAIVE-V1 control (2026-08-04): batch-shuffled c-hat during training —
...
RUN=smolvla_film_0729_prefix_mask1_recal_fromnaive_v1
INIT="$DIR/outputs/smolvla_naive_0729/checkpoints/best/pretrained_model"

[[ -d "$RT/meta" && -d "$RV/meta" && -d "$INIT" ]] || { echo "[fromnaive_v1] inputs missing" >&2; exit 1; }
mkdir -p "$DIR/logs" "$DIR/probes"

env "${RECAL_ENV[@]}" FILM_VARIANT=v1 FILM_MASK_FORCE=1 \
  ...
  > "$DIR/logs/orch_film_0729_fromnaive_v1.out" 2>&1
R=$?; echo "[fromnaive_v1] train rc=$R"
[[ $R == 0 ]] || exit $R

env "${RECAL_ENV[@]}" FILM_MASK_FORCE=1 \
  "$PY" "$DIR/select_best_ckpt.py" --run "$DIR/outputs/$RUN" \
  --val-root "$RV" --repo-id "$REPO_VAL" --prune >> "$DIR/logs/best_0729_fromnaive_v1.out" 2>&1
echo "[fromnaive_v1] best: $(readlink "$DIR/outputs/$RUN/checkpoints/best" 2>/dev/null)"
  ...
  for off in 1 30; do
    for fm in pattern fzdelta; do
      env "${RECAL_ENV[@]}" FILM_MASK_FORCE=1 \
        ...
        > "$DIR/probes/0729_sim_fromnaive_v1_${ck}_off${off}_${fm}.txt" 2>&1
      echo "[fromnaive_v1] sim $ck off$off $fm rc=$?"
    done
  done
  env "${RECAL_ENV[@]}" FILM_MASK_FORCE=1 \
    ...
    --dataset-root "$RV" --repo-id "$REPO_VAL" \
    > "$DIR/probes/0729_state_fromnaive_v1_${ck}_pc_fc.txt" 2>&1
  echo "[fromnaive_v1] pc_fc $ck rc=$?"
  env "${RECAL_ENV[@]}" FILM_MASK_FORCE=1 \
    "$PY" "$DIR/probe_state_authority.py" --swap fcscale --fc-mag 12 --all-episodes --pre-contact 10 \
    --checkpoint "$DIR/outputs/$RUN/checkpoints/$ck" \
    --dataset-root "$RV" --repo-id "$REPO_VAL" \
    > "$DIR/probes/0729_state_fromnaive_v1_${ck}_pc_r12.txt" 2>&1
  echo "[fromnaive_v1] pc_r12 $ck rc=$?"
done
grep -a -H -A4 "── summary" "$DIR"/probes/0729_sim_fromnaive_v1_*.txt | grep -v "^--$"
echo "[fromnaive_v1] DONE"
