...
## 2026-09-14 — 0909 제외 + 구조 위주 재프레이밍 (마감 ICRA 9/15 유지)

- **0909 결과 논문 미사용** (사용자). 0816 다중 백본도 미수록 — GR00T naive 20/20·π0.5 film 실패·ACT film<naive
  로 결과가 섞여 "백본 의존" 논문이 되어버림.
- **뼈대 선택**: 제안 3안(같은 시연 다른 기제 / 계측기 / 레시피) 중 사용자가 **구조 위주**를 택함 —
  "힘 같은 중요 정보를 잃지 않게 하는 효과를 기존 구조 유지하며 얻고, 학습된 모델에 붙일 수 있고, 못 본
  값(더 센 힘·L3)에도 잘 반응". → 기여 = 세 성질(drop-in / retrofittable / monotone-beyond-demos),
  "같은 시연, 다른 기제"는 성질 ③의 해석으로 내려감. 결과 서술은 오프라인 위주, 로봇 결과는 전부 유지하되
  "오프라인에 더해 실로봇에서도 해봤다" 뉘앙스 (사용자: 축소 아님).
- **π0 0729 film-state 복원** (08-13 결정 번복, 사용자): 성질 ①의 유일한 타 백본 근거. π0.5는 전면 미언급.
- **fromnaive 로봇 재실험 안 함** (사용자): 성질 ②는 오프라인+라이브 프로브로만, 9/9는 base-init 인스턴스임을
  §V-D 본문에서 공시.
- 약점 공시(§VII): retrofit·π0 폐루프 없음, 데모 대조 없음, 단일 과제/플랫폼.
