% §III Task and the Failure (~0.75 page). 09-14: fig:setup 참조 제거 — 셋업 사진/헤드캠 프레임 로컬 부재. 0729-pure per Q2: no June depletion sweep /
...
\begin{figure}[t]
\centering
% 09-14: 사용자 제공 사진 (images/Robot_env.jpeg, Battery_box.jpeg) + TikZ 픽토그램 (Battery_stack 대체).
\begin{minipage}[b]{0.29\linewidth}\centering
\includegraphics[width=\linewidth]{figs/setup_robot}\\[-0.2em]{\scriptsize (a)}
\end{minipage}\hfill
\begin{minipage}[b]{0.42\linewidth}\centering
\input{figs/setup_pictogram}\\[0.15em]{\scriptsize (b)}
\end{minipage}\hfill
\begin{minipage}[b]{0.25\linewidth}\centering
\includegraphics[width=\linewidth]{figs/setup_headcam}\\[-0.2em]{\scriptsize (c)}
\end{minipage}
\caption{Setting. (a)~The dual-arm mobile manipulator picking a battery-cell case from a
tray with a suction cup. (b)~The stack height varies across episodes, so the contact plane
moves by centimeters, while the head camera keeps the same pose; a few millimeters of
over-travel take contact force past the $15$\,N limit. (c)~The head-camera view of a tray:
from this range the remaining stack height is visually near-indistinguishable across layers,
so the stop-on-contact transition must be carried by force, not vision.}
\label{fig:setup}
\end{figure}
...
until vacuum seal is confirmed, and lift (Fig.~\ref{fig:setup}a). The cases carry battery
...
heights differing by millimeters---the entire success margin---are visually near-indistinguishable (Fig.~\ref{fig:setup}b,c), while the wrench and seal bit signal the contact transition
...
force level that varies independently of depth. The rationale for this design is given in
Sec.~\ref{sec:discussion}; here it matters that the naive policy and
all FiLM variants are trained on exactly these episodes.

\textbf{The failure.}
Deployed on the robot, the naive policy---a VLA fine-tuned on these demonstrations, with the
full wrench and seal bit in its state (backbone in Sec.~\ref{sec:offline})---fails every evaluation trial
...
that observably announces it, is the transition the policy does not perform. Sec.~\ref{sec:method}
describes the pathway that changes this and the probes we use to measure the change.
