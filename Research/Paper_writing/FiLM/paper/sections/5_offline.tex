...
\addlinespace
conditioned policy & grounded pathway & \makecell{$\mathbf{+0.9}$\\{\scriptsize own $-7.8$}} & \makecell{$+1.57$ (94\%)\\{\scriptsize own $-1.67$}} & \makecell{$+1.20\rightarrow\mathbf{+4.40}$ (31$\rightarrow$\textbf{115\%}) $\uparrow$\\{\scriptsize own $-3.84$}} & \textbf{max 9.8\,mm} \\
\bottomrule
\end{tabular}
\end{table*}
...
\addlinespace
FiLM, mask, shuffled \chat{} & ungrounded pathway & \makecell{$-0.2$\\{\scriptsize own $-8.2$}} & \makecell{$-0.06$ (0\%)\\{\scriptsize own $-1.35$}} & \makecell{$-0.06\rightarrow-0.11$ ($\approx$0)\\{\scriptsize own $-1.35$}} & \textbf{max 57.2\,mm} \\
\bottomrule
\end{tabular}
\end{table*}
...
Fine-tuning $\pi_0$~\cite{pi0} and $\pi_{0.5}$~\cite{pi05} ($3$B-class, ${\sim}8\times$
SmolVLA) on the same demonstrations reproduces the failure at scale, in two forms.
$\pi_{0.5}$ shows the template signature outright: a strong on-manifold transplant response
($65\%$) whose dose-response saturates shallowly ($17\%\rightarrow25\%$). $\pi_0$'s response
rises with dose ($20\%\rightarrow80\%$) yet stays below full cancellation---and below what
stopping a stiff press requires. Both collapse in the seal-never press: no episode settles,
all still descending when the step budget ends, at max penetrations of $48$ and $19.6$\,mm
(versus $14.9$ for the naive SmolVLA, and $9.8$, settled, for the conditioned policy), at
comparable imitation error ($0.89$--$1.03$\,mm per step). Neither parameter count nor
architecture converts raw force access into force-bound braking; what changes the class is
the conditioning pathway, not scale.
