...
information) and \emph{retrofittable} (attachable to a trained policy); it is also
\emph{measurable}: because it is explicit and unique, every component makes one number
...
\caption{The FiLM policy. The raw wrench does not enter the policy state; force reaches
...
finding. The FiLM policies in this paper are separately trained instances of the same
design: the robot picks (Sec.~\ref{sec:robot}) and the replication pair quoted in
Sec.~\ref{sec:offline-quintuple} use an earlier calibration; the ablations and the live
...
untouched: at the moment the pathway is attached, the FiLM policy \emph{is} the
already-trained policy, and can be fine-tuned in place from there. Every offline FiLM
...
along the approach and apply (P1)--(P3) interventions to \emph{two} policies---the FiLM
