...
\textbf{Why the demonstrations must be anticipatory.}
...
gating, not from a reflex.

% 08-13 사용자 결정: "Grounding is not free" 문단 삭제 (π0 state-site 이식 결과는 미보고 —
% EVIDENCE §3.9 아카이브). §IV 각주의 Sec.VII 참조 문장도 함께 제거됨.
\textbf{A computed condition is an operational knob.}
Because \chat{} is computed rather than learned, the pathway creates knobs the naive policy lacks. Force-torque baselines drift
(${\sim}1$\,N within a day, pose-sensitive); the FiLM policy re-anchors its offsets at a
hover pose each run---a deployment-time correction with no retraining---while the baseline's
force statistics are frozen in its normalization.
...
seven policies across two backbones). The retrofitted instance and the $\pi_0$ instance were
not deployed in closed loop, and the press-retreat demonstrations are a precondition we do not
ablate here. The press simulation depends on a linear force model;
...
the state, and testing whether force-fusion architectures that keep the raw signal alongside a
dedicated route~\cite{forcevla,forceflow} also bypass it.
