...
\emph{computed} from signals the naive policy already receives and injected through FiLM
modulation~\cite{perez2018film}, and the raw wrench is masked from the state, so \chat{}
...
\item \textbf{From force runaway to force regulation, with the same demonstrations.} A
  grounded FiLM bottleneck---\chat{} computed from signals the policy already receives,
